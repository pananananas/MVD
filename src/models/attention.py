...
class ImageCrossAttentionProcessor(nn.Module):
    def __init__(
        ...
        self.to_out_ref = nn.ModuleList(
            [nn.Linear(self.inner_dim, query_dim, bias=True), nn.Dropout(dropout)]
        )
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if ref_hidden_states is None or self.name not in ref_hidden_states:
            if not hasattr(self, "_logged_ref_missing"):
                self._logged_ref_missing = set()
            if "validation" not in self._logged_ref_missing:
                log_debug(
                    debug_log_file_path,
                    f"{self.name}: ref_hidden_states is None or key missing. Skipping ref attention.",
                )
                self._logged_ref_missing.add("validation")
            return original_output

        reference_states_input = ref_hidden_states[
            self.name
        ]  # Renamed to avoid confusion

        log_debug(
            debug_log_file_path, f"{self.name}: Received ref_hidden_states during call"
        )
        log_debug(
            debug_log_file_path,
            f"{self.name}: reference_states_input (raw) stats: mean={reference_states_input.mean():.4f}, std={reference_states_input.std():.4f}",
        )

        with torch.no_grad():
            reference_states = reference_states_input.clone()
            reference_states = reference_states - reference_states.mean(
                dim=(0, 1), keepdim=True
            )
            ref_std_tensor = torch.clamp(
                reference_states.std(dim=(0, 1), keepdim=True), min=1e-6
            )
            reference_states = reference_states / ref_std_tensor * 0.5

        log_debug(
            debug_log_file_path,
            f"{self.name}: reference_states (normalized for K,V) stats: mean={reference_states.mean():.4f}, std={reference_states.std():.4f}",
        )
        ...
        reference_states = self._adapt_reference_features(
            reference_states, self.query_dim
        )
        ...
        log_debug(
            debug_log_file_path,
            f"{self.name}: query stats: mean={query.mean():.4f}, std={query.std():.4f}",
        )
        log_debug(
            debug_log_file_path,
            f"{self.name}: key stats (after to_k_ref): mean={key.mean():.4f}, std={key.std():.4f}",
        )
        log_debug(
            debug_log_file_path,
            f"{self.name}: value stats (after to_v_ref): mean={value.mean():.4f}, std={value.std():.4f}",
        )
        ...
        if input_ndim == 4:
            batch_size, channel, height, width = hidden_states.shape
            ref_hidden_states_output = ref_hidden_states_output.transpose(
                -1, -2
            ).reshape(batch_size, channel, height, width)

        log_debug(
            debug_log_file_path,
            f"{self.name}: ref_hidden_states_output (attn out after LayerNorm) stats: mean={ref_hidden_states_output.mean():.4f}, std={ref_hidden_states_output.std():.4f}",
        )
        ...
        log_debug(
            debug_log_file_path,
            f"{self.name}: scaled_ref_contribution ({self.ref_scale_val=}) stats: mean={scaled_ref_contribution.mean():.4f}, std={scaled_ref_contribution.std():.4f}",
        )
        ...
        log_debug(
            debug_log_file_path,
            f"{self.name}: combined_output stats: mean={combined_output.mean():.4f}, std={combined_output.std():.4f}",
        )
    ...
    def _adapt_reference_features(self, reference_states, target_dim):
        if reference_states.ndim == 4:
            ...
            reference_states = reference_states.permute(0, 2, 3, 1).reshape(
    ...
    def load_original_weights(self, attn_module):
        with torch.no_grad():
            ...
            if k_out == ok_out and k_in == ok_in:
                ...
                if k_in >= ok_in:
                    self.to_k_ref.weight[:, :ok_in].copy_(
                        orig_k[: min(k_out, ok_out), :]
                    )
                    if k_in > ok_in:
            ...
            if v_out == ov_out and v_in == ov_in:
                ...
                if v_in >= ov_in:
                    self.to_v_ref.weight[:, :ov_in].copy_(
                        orig_v[: min(v_out, ov_out), :]
                    )
                    if v_in > ov_in:
