...
class CameraEncoder(nn.Module):
    ...
    def compute_relative_transform(
        self, source_camera: torch.Tensor, target_camera: torch.Tensor
        ...
        source_to_target_T = target_T - torch.bmm(
            source_to_target_R, source_T.unsqueeze(2)
        ).squeeze(2)

        return {"R": source_to_target_R, "T": source_to_target_T}
    ...
    def to(self, *args, **kwargs):
        ...
        if device is not None:
            ...
            self.translation_encoder = self.translation_encoder.to(
    ...
    def positional_encoding(self, x: torch.Tensor) -> torch.Tensor:
        ...
        freqs = torch.exp(
            torch.linspace(0.0, np.log(self.max_freq), self.pos_enc_dim, device=device)
        )
        ...
        weight = torch.randn(
            self.output_dim, encoding.shape[-1], device=device
        ) / np.sqrt(encoding.shape[-1])
    ...
    def apply_modulation(
        self, hidden_states, modulator_name: str, camera_embedding: torch.Tensor
    ):
        if isinstance(hidden_states, tuple):
            modulated_main = self.apply_modulation_to_tensor(
                hidden_states[0], modulator_name, camera_embedding
            )
            return (modulated_main,) + hidden_states[1:]
        else:
            return self.apply_modulation_to_tensor(
                hidden_states, modulator_name, camera_embedding
            )
    ...
    def apply_modulation_to_tensor(self, tensor, modulator_name, camera_embedding):
        ...
        if camera_embedding is None:
            ic(
                f"WARNING: apply_modulation called for {modulator_name} but camera_embedding is None. Skipping modulation."
            )
            return tensor
