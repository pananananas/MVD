    ...
    def _register_hooks(self):
        ...
        for i, block in enumerate(self.unet.down_blocks):
            if hasattr(block, "attentions"):
                logger.debug(
                    f"Registering hooks for down_block_{i} (has {len(block.attentions)} attention layers)"
                )
                for j, layer in enumerate(block.attentions):
                    name = f"down_block_{i}_attn_{j}"
                    if name not in registered_layers:
                        hook = layer.register_forward_hook(
                            lambda module, input, output, name=name: self._hook_fn(
                                name, output
                            )
                        )
        ...
        for i, block in enumerate(self.unet.up_blocks):
            if hasattr(block, "attentions"):
                logger.debug(
                    f"Registering hooks for up_block_{i} (has {len(block.attentions)} attention layers)"
                )
                for j, layer in enumerate(block.attentions):
