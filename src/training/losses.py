...
def compute_losses(noise_pred, noise, denoised_latents=None, target_latents=None, vae=None, perceptual_loss_fn=None, ssim_loss_fn=None, config=None):
    """
    Simplified loss function focusing only on noise prediction
    
    Args:
        noise_pred: Predicted noise (4-channel latent)
        noise: Ground truth noise (4-channel latent)
        [other args are kept for API compatibility but unused]
    """
    # Use only noise prediction loss (MSE between predicted and actual noise)
    noise_loss = F.mse_loss(noise_pred, noise)
    
    # Return other values as zero tensors to maintain API compatibility
